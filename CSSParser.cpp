...
#define PopOperator(node) ASTNode* node = operatorStack.top();\
    ...
    CSSParser* CSSParser::getParser()
    {
        static CSSParser* parser = 0;
        if (!parser) {
    ...
    bool CSSParser::parser(const std::string &cssFile)
    {
        if (cssFile.empty()) {
            return false;
        }
        ...
        m_selectors.clear();
        m_lexer->SetBufferSource(cssFile);
            ...
            switch (m_status) {
                ...
                case STARTBLOCK: {
                    ...
                    if (token->type == BLOCKEND) {
                        ...
                        while (it != end) {
                            std::stack<CSSParser::ASTNode *> *result = new std::stack<CSSParser::ASTNode *>;
                            MLRtranverseAST(*it, TreeTranverseCreateExpressionAction, result);
    ...
    void CSSParser::buildReversePolishNotation(std::stack<ASTNode *> &operatorStack, std::stack<ASTNode *> &operandStack)
    {
    ...
    void CSSParser::LMRtranverseAST(future::CSSParser::ASTNode *root, future::CSSParser::treeTranverseAction action)
    {
        if (!root) {
            return;
        }
    ...
    void CSSParser::RMLtranverseAST(future::CSSParser::ASTNode *root, treeTranverseAction action)
    {
        if (!root) {
            return;
        }
    ...
    void CSSParser::LRMtranverseAST(future::CSSParser::ASTNode *root, future::CSSParser::treeTranverseAction action)
    {
        if (!root) {
            return;
        }
    ...
    CSSParser::ASTNode* TreeTranverseCreateExpressionAction(std::stack<CSSParser::ASTNode *>* stack)
    {
        using ASTNode = CSSParser::ASTNode;
        if (stack->size() != 3) {
            return 0;
        }
    ...
    void CSSParser::pushSign(std::stack<Selector *> &stack, SignSelector::SignType type)
    {
        if (!stack.size()) {
            return;
        }
    ...
    void CSSParser::pushOperatedElement(std::stack<ASTNode *> &stack, future::Selector *head)
    {
